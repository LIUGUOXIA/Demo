...
/* 按键口对应的RCC时钟 */
#define RCC_ALL_KEY 	(RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOC | RCC_APB2Periph_GPIOF | RCC_APB2Periph_GPIOG)
...
*/
	...
	static uint8_t IsKeyDown2(void) {if (GPIO_ReadInputDataBit(GPIOC,GPIO_Pin_13) == 1) return 1;else return 0;}
	static uint8_t IsKeyDown3(void) {if ((GPIO_PORT_K3->IDR & GPIO_PIN_K3) == 1) return 1;else return 0;}
	static uint8_t IsKeyDown4(void) {if ((GPIO_PORT_K4->IDR & GPIO_PIN_K4) == 0) return 1;else return 0;}
	static uint8_t IsKeyDown5(void) {if ((GPIO_PORT_K5->IDR & GPIO_PIN_K5) == 0) return 1;else return 0;}
	static uint8_t IsKeyDown6(void) {if ((GPIO_PORT_K6->IDR & GPIO_PIN_K6) == 0) return 1;else return 0;}
	static uint8_t IsKeyDown7(void) {if ((GPIO_PORT_K7->IDR & GPIO_PIN_K7) == 0) return 1;else return 0;}
...
Key_Table g_stKeyTable[TABLE_SIZE] =
{
		{0, 9, 1},
		{1, 0, 2},
		{2, 1, 3},
...
uint16_t g_usCurIndex = 0;
uint16_t g_usLastIndex = 0;
...
void Key_5msMainFunction(void)
{
	...
	if(KeyVal == 1)
	{
	  ...
	  //	bsp_SetTIMOutPWM(GPIOB, GPIO_Pin_5, TIM3, 2, 15000, duty * 10000/250);	
	    bsp_SetTIMOutPWM(GPIOB, GPIO_Pin_0, TIM3, 3, 15000, duty * 10000/250);	
		//	bsp_SetTIMOutPWM(GPIOB, GPIO_Pin_1, TIM3, 4, 15000, duty * 10000/250);	
		//LED_SetStatus(LED_DETECT_WORK,1);
		g_usCurIndex = g_stKeyTable[g_usCurIndex].up;
	}
	else if(KeyVal == 4)
	{
		 Beep_SetStatus(BEEP_DOING_WORK, 1);       //beep不响
		LED_SetStatus(LED_DETECT_WORK,0);
			duty-=10;
		if(duty<0)
		{
			duty = 255;
	  ...
	  //	bsp_SetTIMOutPWM(GPIOB, GPIO_Pin_5, TIM3, 2, 15000, duty * 10000/250);	
			bsp_SetTIMOutPWM(GPIOB, GPIO_Pin_0, TIM3, 3, 15000, duty * 10000/250);	
		//	bsp_SetTIMOutPWM(GPIOB, GPIO_Pin_1, TIM3, 4, 15000, duty * 10000/250);	
		g_usCurIndex = g_stKeyTable[g_usCurIndex].down;
	}
	else
	{
	...
	if(g_usCurIndex != g_usLastIndex)
	{
		g_usLastIndex = g_usCurIndex;
		printf("g_usCurIndex=%d\r\n", g_usCurIndex);
	}
...
//	if(KeyVal != 0)
//	{
//		printf("按键 is ok KeyVal=%d",KeyVal);
//	}
