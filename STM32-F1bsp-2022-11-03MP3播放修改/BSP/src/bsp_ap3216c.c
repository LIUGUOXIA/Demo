...
{
    uint8_t IntStatus;
    read_regs(AP3216C_SYS_INT_STATUS_REG, 1, &IntStatus);          /* 读中断状态寄存器 */
...
void AP_INT_Config(void)
{
    ...
    AP_INT_GPIO_APBxClkCmd(AP_INT_GPIO_CLK,ENABLE);           /*开启按键GPIO口的时钟*/
    GPIO_InitStructure.GPIO_Pin = AP_INT_GPIO_PIN;            /* 选择按键1的引脚 */ 
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN_FLOATING;	    /* 设置引脚为输入模式 */ 		
    GPIO_Init(AP_INT_GPIO_PORT, &GPIO_InitStructure);        /* 使用上面的结构体初始化按键 */
...
*/
  ...
  uint16_t PS;
  uint16_t IR;
void ap3216c_10msMainFunction(void)
{
...
uint16_t Ap3216c_getDistance(void)
{
//	uint16_t ALS;
//	ALS = ap3216c_read_ambient_light();
	return PS;
...
uint16_t Ap3216c_Light_intensity(void)
{
//	uint16_t ALS;
//	ALS = ap3216c_read_ambient_light();
	return ALS;
