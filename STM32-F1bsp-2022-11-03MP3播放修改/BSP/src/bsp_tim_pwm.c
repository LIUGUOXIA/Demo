...
*********************************************************************************************************
*/
uint32_t bsp_GetAFofTIM(TIM_TypeDef* TIMx)
{
	uint32_t ret = 0;
	...
	*/
	if (TIMx == TIM1)
	{
		ret = GPIO_PartialRemap_TIM1;
	}
	else if (TIMx == TIM2)
	{
		ret = GPIO_PartialRemap1_TIM2;
	}
	/* 下面是 APB1时钟 */
	else if (TIMx == TIM3)
	{
		ret = GPIO_PartialRemap_TIM3;
	}
	else if (TIMx == TIM4)
	{
		ret = GPIO_Remap_TIM4;
...
void bsp_ConfigTimGpio(GPIO_TypeDef* GPIOx, uint16_t GPIO_PinX, TIM_TypeDef* TIMx, uint8_t _ucChannel)
{
	...
	GPIO_PinRemapConfig( bsp_GetAFofTIM(TIMx), ENABLE); 	
	 ...
	 uint32_t _ulFreq, uint32_t _ulDutyCycle)
{
	...
	else if (_ulDutyCycle == 10000)    //_ulDutyCycle : PWM信号占空比，单位：万分之一
	{
		TIM_Cmd(TIMx, DISABLE);		/* 关闭PWM输出 */
	...
	if (_ulFreq < 100)       //_ulFreq : PWM信号频率，单位Hz 
	{
		usPrescaler = 10000 - 1;					/* 分频比 = 10000 */                     //定时1ms，由于1ms=1us*1000,那么预装载值就是1000-1
		usPeriod =  (uiTIMxCLK / 10000) / _ulFreq  - 1;		/* 自动重装的值 */       //  占空比=TIM /周期      频率=1 / 周期
	}
