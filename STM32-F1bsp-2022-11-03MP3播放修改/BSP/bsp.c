...
void bsp_Init(void)
{
	/*
		...
		系统时钟缺省配置为192MHz，如果需要更改，可以修改 system_stm32f4xx.c 文件
	*/
	/* 优先级分组设置为4 */
...
{	
	UART_5msMainFunction();
	Led_5msMainFunction();
	Beep_10msMainFunction();      //此任务打开的话，蜂鸣器调用需要使用接口Beep_SetStatus  其他直接控制硬件的会被影响不发声
	Irda_10msMainfunction();
  ADC_20msMainfunction();
	//ap3216c_10msMainFunction();
