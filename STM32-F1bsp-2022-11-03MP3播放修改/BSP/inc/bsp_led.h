...
typedef struct
{
		uint16_t Duty_ratio;         /*占空比*/
		uint16_t flag;              /*呼、吸标志位，flag=0为呼，flag=1为吸*/
		uint16_t count;             /* 次数计数器 */
		uint16_t Time;             /* 半个呼吸时间 */
	  void (*PwmOut_Led)(uint32_t _ulDutyCycle);    /* 设置引脚输出的PWM信号的频率和占空比的回调函数指针 主要用于控制呼吸灯 */
...
void Breathing_lamp_10msMainFunction(void);
